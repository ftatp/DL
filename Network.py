...
##############################################################################################################
    def __init__(self, input_size, output_size, num_of_hidden_layers, weight_init_std=0.01):
        ...
        weight = np.random.randn(next_input_size, output_size)
        network['weights'].append(weight)
    ...
    def set_network_by_file(network_file):
    ...
    def set_network_training_methods(activation_func=activationFuncs.identity, output_func=activationFuncs.identity, loss_func=lossFuncs.mean_squared_error)
        #########################################
        ## Set activation, output, loss functions
        #########################################
